...
#treatment info
insertQuery = "INSERT INTO production_treatment (treat_cpt, treat_human_name) VALUES (%s, %s)"
treatRowCount = 0
for index, row in cpt_codes_1k.iterrows():
    ...
    if treatRowCount == 75:
        break
df_gcp = pd.read_sql_query("SELECT * FROM production_treatment", db_gcp)

#social info 
insertQuery = "INSERT INTO production_social (social_lonic, social_human_name) VALUES (%s, %s)"
lonicRowCount = 0
for index, row in lonic_codes_1k.iterrows():
    ...
    if lonicRowCount == 75:
        break
df_gcp = pd.read_sql_query("SELECT * FROM production_social", db_gcp)
